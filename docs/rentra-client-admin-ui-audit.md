...
| G13 | Missing / P1 | No mounted admin booking amendment/owner-cancellation case workflow | Previewed, idempotent case resolution using accepted rules and available inventory | CP14 — **complete, 27 September 2026**: owner/admin booking cases for exact visits, assignment, audience-scoped updates, previewed one-time resolution reusing customer cancellation with capped Test refunds, non-reserving change checks; 34 browser/API checks plus 6 failure-path checks ([handoff](rentra-client-admin-part14.md)) |
