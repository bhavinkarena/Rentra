...
- **Updated 14 September 2026 by the [Parts 01–08 UX review](rentra-customer-parts01-08-ux-review.md) follow-up:** primary navigation is now Explore/Saved plus a customer-aware Account or Log in control; Bookings was removed from it while there is no booking history to show. `/bookings` remains a routable session-protected page for Part 13 and keeps its availability notice, but nothing in the app links to it. Owner entry moved to secondary navigation and the footer. `/account/payment-methods` is no longer linked from `/account`; the account page states the same unavailability inline.
