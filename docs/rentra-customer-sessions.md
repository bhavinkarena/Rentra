...
- **Completed parts:** 01–02 — COMPLETE (12 September 2026); 03 — COMPLETE (13 September 2026).
- **Next planned part:** 04 — authoritative quotes and inventory rules.
...
- **Next-session notes:** implement Part 04 only. Read both runbooks and preserve the custom exclusion, financial triggers and internal accounting view when generating migrations. Introduce the authoritative quote contract, owner slot schedules/buffers and listing-first inventory lock discipline; remediate legacy active intervals and owner blocks before switching availability authority. Financial writers must follow listing/booking-order locks with payment-order locks and handle whole-transaction serialization retries. Part 11 adds customer holds/confirmation; Parts 20–22 add real providers and settlement/recovery. Apply additive migrations before enabling readers of their new columns/view, and never use the configured database as disposable.
