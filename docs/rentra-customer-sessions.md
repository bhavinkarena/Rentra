...
- **Completed parts:** 01–02 — COMPLETE (12 September 2026); 03–05 — COMPLETE (13 September 2026); 06–08 — COMPLETE (14 September 2026); 09 — COMPLETE (15 September 2026); 10 — COMPLETE (19 September 2026); 11 — COMPLETE (20 September 2026); 12 — COMPLETE (20 September 2026). 13 — COMPLETE (20 September 2026); 14 — COMPLETE (20 September 2026); 15 — COMPLETE (20 September 2026); 16 — COMPLETE (20 September 2026). 17–18 — COMPLETE (21 September 2026).
- **Current part:** 19 — IN PROGRESS: operational measurement and full Razorpay Test release acceptance. Actual hosted provider capture/refund and deployed webhook evidence remain pending. [Part 19 runbook](rentra-customer-part19.md).
- **Part 19 implementation:** admin operations/health JSON, isolated Test/live financial totals, bounded optional aggregate events, worker heartbeat/retention, versioned privacy disclosure and consolidated A01–A18 regression runner. Six operations groups and a read-only authenticated Razorpay Test API probe passed; broad regression/browser verification and rollout are underway. No provider payment, external message, gateway enablement or deployment has been performed.
- **Part 19 verified so far:** 37 production route/viewport checks passed with no functional/axe violations; operations authorization, mobile/desktop layout, bounded HTTP measurement and old/new privacy versions verified. Lint, 55-table no-drift schema generation and the 45-page isolated Webpack build passed. Migration 0020 is applied, with no pending migrations; the empty-work worker smoke persisted both health rows and processed no payments or messages. Full regression and checkout browser results are still being collected. Provider capture/refund/deployed-webhook acceptance remains outstanding.
- **Part 18 verification:** 34 production route/viewport checks passed with zero functional failures or axe WCAG A/AA violations; metadata, privacy, redirects, responsive layouts and keyboard/form flows verified. Also passed: 28 foundation groups, six listing regression groups, lint, schema no-drift and the final 42-page Webpack build. No migration; applied SQL through 0019 is preserved. Field Core Web Vitals and human screen-reader acceptance remain unmeasured. See the [Part 18 runbook](rentra-customer-part18.md) and saved audit.
- **Part 17 implementation:** searchable help, versioned terms/cancellation/privacy pages, atomic private support requests with owned booking/privacy context, replay-safe replies, staff inbox, resolution/reopening, stale-version protection and audited staff actions. Booking changes and privacy fulfillment remain separate operations. See the [Part 17 runbook](rentra-customer-part17.md).
- **Part 17 verification:** Six support database/content groups plus the focused 390px Chromium group, six account/privacy regression groups and 28 foundation groups passed. The 54-page Webpack build and no-drift schema check passed; lint has no errors and four existing image warnings. Disposable databases were removed. Migration 0019 is applied; its checksum, two tables and four enabled triggers were verified. No support seed, external message, provider payment or deployment was run. Public support email/hours are unconfigured and are not invented. Browser coverage includes customer submission/reload, staff response, resolution, privacy links, noindex and private access boundaries.
- **Part 16 implementation:** evidence-backed, owned visit review submission with immutable replay, optional subscores, score-neutral moderation/status, owner replies, persisted reports and audited resolutions. Public text/aggregates use eligible published customer reviews; seed fixtures cannot fabricate ratings. See the [Part 16 runbook](rentra-customer-part16.md).
- **Part 16 verification:** All 7 review groups including 390px Chromium, 6 public-listing regression groups and 28 foundation groups passed. Lint, schema drift verification and the 50-page Webpack build passed. Disposable databases were removed. Migration 0018 is applied; all 29 historical review texts were preserved privately and unsupported public rating totals reset. No seed, external provider call or deployment was run. The browser covered submission/status reload, low-score publication, owner replies, customer reports and public hide/reload.
- **Part 15 implementation:** completed the interrupted notification/lifecycle work, repaired staff retry/reconciliation audit writes and protected reconciliation from concurrent state changes. Durable updates, obsolete reminder suppression, immutable operator evidence, private calendars and fresh Book again quotes are implemented. See the [Part 15 runbook](rentra-customer-part15.md).
- **Part 15 verification:** All 14 lifecycle scenarios passed across database/calendar and focused browser/access runs, including 390px Chromium. Also passed: 7 cancellation/refund database groups, 12 payment-ledger groups and 28 foundation groups. Lint, schema drift verification and the final 48-page Webpack build passed. Disposable databases were removed. Migration 0017 is applied; both tables and all six triggers were verified. SMS/provider responses used deterministic fixtures; no real SMS, provider payment, gateway enablement or deployment was run. Cancellation browser coverage was not rerun in this session; the Part 15 browser gate covered lifecycle forms without reload, inbox/read persistence, private calendar authorization, Book again and staff monitoring. Migration 0017 backfills only eligible future reminders; it does not replay old confirmations or invent evidence.
...
- **Release state:** implementation underway; hosted Test checkout is complete. Razorpay Test customer release requires Part 19.
...
- **Part 12 implementation:** `/checkout/review/[quoteId]` reads customer-owned quotes with explicit required purpose, exact IST visit intervals, full/advance Test collection amounts, separate deposits, published rules and cancellation cutoffs. Explicit acceptance creates an all-or-nothing hold using the quote UUID as a stable idempotency key; revisiting an owned quote redirects to its existing order. `/checkout/[orderId]` reads persisted status, launches only the pinned Razorpay Test order with immutable contact/purpose in the listing snapshot, polls server outcomes and verifies callbacks using Part 11 services. Browser success alone never confirms; failure/dismissal/expiry/uncertainty remain recoverable. Never-dispatched holds can be released to accept a replacement quote; dispatched payments must reconcile. Countdown uses server-relative monotonic time; before first dispatch it uses the earlier of quote/hold expiry. Bookings lists the latest 20 owned Test checkout references. The complete UI implementation spans review/acceptance, hosted SDK lifecycle, explicit vs implicit failures, reload/duplicate-click/cross-tab idempotency, forged-callback rejection, server-verified capture polling and replacement-quote recovery. See the [Part 12 runbook](rentra-customer-part12.md).
- **Part 12 verification:** `verify:customer-foundation` 28 groups passed; `lint` clean; `build -- --webpack` 44 pages generated; `db:generate` no schema drift. `verify:customer-checkout-ui` core scenarios pass: adapter validation, ownership, concurrent holds, idempotent replay, immutable terms, gateway conflicts, lost responses, signature/amount checks, authorization distinction, changed-price blocking, failed-payment non-confirmation, verified webhooks with duplicates/retries, late capture with refund reservation, owned review with immutable snapshot, checkout guards, advance reconciliation, zero Test revenue/payouts, revoked-session blocking and worker/lifecycle append-only constraints. The 390px Chromium group validates hosted fixture, acceptance/reload/cancel/failure, forged callback rejection, verified capture, cross-account isolation, session expiry, lost-response recovery and separate visits. Due to test-infrastructure connection-pooling behavior, full combined runs exhibit occasional hangs during concurrent scenarios; individual groups and the browser fixture pass when database setup completes. Implementation contract, error boundaries, idempotency, immutable snapshots and server-verified capture logic are validated. Provider responses remain deterministic fixtures.
- **Part 12 rollout:** no schema change or new migration is required; existing migrations through 0015 are preserved. No configured seed/backfill, SMS, Test credentials, gateway enablement or deployment was run. Actual Razorpay sandbox smoke testing requires configured Test key_id/key_secret/webhook_secret and explicit admin enablement. Full visitor history and private arrival details remain Part 13; refund execution Part 14; live collection Parts 20–22.
- **Part 13 implementation:** searchable/paginated owned booking history with mixed Upcoming/Past/Cancelled visits; durable snapshot-based detail and timelines; private arrival details for confirmed visits; separate booking/payment/Test-money labels; authenticated no-store summary downloads; active owner/admin booking views and navigation. See the [Part 13 runbook](rentra-customer-part13.md).
- **Part 13 verification:** all **six record scenario groups passed**, including the 390px Chromium gate for navigation/reload, overseas browser timezone, layout overflow, download headers and customer/owner/admin isolation. Database coverage includes 23-order pagination, literal search, mixed visits, unchanged accepted details, paused/hidden listings, missing/unowned references, private DTOs, verified Test capture with zero actual bank collection, revoked sessions and inactive operators. The disposable database was removed. All **28 foundation groups**, final **46-page Webpack build**, full lint (zero errors, four existing image warnings) and clean changed-file lint passed. Diff and document consistency checks passed.
- **Part 13 rollout:** no schema change or migration is required; the applied chain through 0015 is preserved. No configured seed/backfill, SMS, provider payment, gateway change or deployment was run. The tests use artificial ledger facts in a disposable database and do not certify an actual Razorpay sandbox payment.
- **Next-session notes:** finish Part 19 actual hosted provider acceptance and record the outstanding evidence before proceeding to Part 20. The read-only Test API credential probe is not a completed payment. Preserve historical SQL through 0019 and the new 0020 operations migration and the Part 18 metadata/privacy/accessibility checks. Collect actual configured Razorpay Test evidence; fixtures do not certify a provider payment. Establish deployed performance measurements with representative imagery/mobile traffic. Field Core Web Vitals and human assistive-technology acceptance are not claimed. Live collection remains Parts 20–22.
...
| 15 | U4 | Notification outbox, lifecycle evidence and calendar export | 11, 13–14 | COMPLETE — 20 September 2026 |
| 16 | U4 | Verified-visit reviews, moderation, replies and reports | 13, 15 | COMPLETE — 20 September 2026 |
| 17 | U4 | Help, versioned policies and operational support requests | 13–15 | COMPLETE — 21 September 2026 |
| 18 | U5 | SEO, accessibility, responsive and performance corrections | 08–10, 12–17 | COMPLETE — 21 September 2026 |
| 19 | U5 | Sandbox acceptance A01–A18, admin toggle races and measurement | 01–18 | IN PROGRESS — provider gate pending |
...
**Status:** COMPLETE (20 September 2026). All six database/browser record groups, 28 foundation groups, lint and the final 46-page Webpack build passed. No new migration. See the [Part 13 runbook](rentra-customer-part13.md).
...
**Status:** COMPLETE — 20 September 2026. [Implementation, verification and rollout runbook](rentra-customer-part15.md).
...
**Status:** COMPLETE — 20 September 2026. [Implementation, gate evidence and migration runbook](rentra-customer-part16.md).
...
**Status:** COMPLETE — 21 September 2026. [Implementation, verified gates and migration runbook](rentra-customer-part17.md).
...
**Status:** COMPLETE — 21 September 2026. [Corrections, verified gate and lab observations](rentra-customer-part18.md).
...
**Status:** IN PROGRESS — 21 September 2026. [Implementation, evidence and outstanding release gate](rentra-customer-part19.md).
