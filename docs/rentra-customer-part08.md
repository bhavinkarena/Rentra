...
- Single and consecutive modes use date inputs. A consecutive range expands to separate visits, not continuous possession. The richer listing date picker remains Part 10.
- **Updated 14 September 2026 by the [Parts 01–08 UX review](rentra-customer-parts01-08-ux-review.md) follow-up:** separate mode no longer accepts a typed comma-separated string. Each visit date is its own `type="date"` input in a removable chip, with Add date up to ten visits; the joined `dates` value is submitted through a hidden field, so the URL contract and `parseDiscoveryQuery` are unchanged. City and area are chosen together in one city-grouped Where control, removing the city-only submission that previously populated Area. Budget, property type, cancellation and amenities moved into a collapsed Filters disclosure that opens automatically when any of them is active. Sort moved beside the result count. Chips now display readable names and values (`Surat`, `Up to ₹8,000`, `Swimming pool`, `Full day`) instead of filter keys, and clearing the city chip also clears its dependent area.
