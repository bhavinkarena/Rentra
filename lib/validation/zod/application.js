...
export const kycSchema = z
  ...
  .refine((d) => d.kycDocType !== 'pan' || PAN_RE.test(d.panNumber ?? ''), {
    message: 'PAN looks like ABCDE1234F',
    path: ['panNumber'],
  })
  .refine((d) => d.kycDocType !== 'aadhaar' || /^\d{4}$/.test(d.aadhaarLast4 ?? ''), {
...
export const payoutSchema = z
  ...
  .refine((d) => d.method !== 'upi' || UPI_RE.test(d.upiId ?? ''), {
    message: 'UPI ID looks like yourname@bank',
    path: ['upiId'],
  })
  .refine((d) => d.method !== 'bank' || /^\d{9,18}$/.test(d.accountNumber ?? ''), {
    message: 'Enter a valid account number',
    path: ['accountNumber'],
  })
  .refine((d) => d.method !== 'bank' || IFSC_RE.test(d.ifsc ?? ''), {
    message: 'IFSC looks like SBIN0001234',
    path: ['ifsc'],
  });

export const consentSchema = z.object({
  acceptTerms: z.literal('on', { message: 'You need to accept the terms to continue' }),
  declareEntitled: z.literal('on', {
    message: 'Please confirm you are entitled to let the property',
  }),
});
