...
export const listingDraftSchema = z.object({
  ...
  fulfilment: z.enum(['visit_site', 'pickup_from_owner', 'delivered']).default('visit_site'),
...
export const basicsSchema = z.object({
  ...
  description: z
    .string()
    .trim()
    .min(40, 'Tell guests a bit more — 40 characters minimum')
    .max(4000),
...
export const pricingSchema = z
  .object({
    day_weekday: rupees,
    day_weekend: rupees,
    night_weekday: rupees,
    night_weekend: rupees,
    full_day_weekday: rupees,
    full_day_weekend: rupees,
...
export const ownershipDocSchema = z.object({
  ...
  issuedAt: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, 'When was it issued?')
    .optional()
    .or(z.literal('')),
});
