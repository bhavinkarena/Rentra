...
const enabledSlot = z
  ...
  .strict()
  .refine((value) => value.includedGuests <= value.capacity, 'Included guests exceed capacity');
...
export const bookingConfigSchema = z
  ...
  .superRefine((config, ctx) => {
    ...
    for (const [name, schedule] of Object.entries(config.slots)) {
      ...
      try {
        visitInterval({ date: '2030-01-01', slot: name, schedule, timeZone: config.timeZone });
      } catch (error) {
        ctx.addIssue({ code: 'custom', path: ['slots', name], message: error.message });
      }
    }
  });
