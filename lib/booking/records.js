...
export async function readBookingRecord(database, actor, orderId, env = process.env) {
  ...
  return database.begin(async tx => {
    ...
    const rows = await tx`SELECT id,reference,state,local_day,day,slot,guests,starts_at,ends_at,hours_known,
      amount_rent_minor,amount_fee_minor,amount_deposit_minor,created_at,confirmed_at,cancelled_at,updated_at
    ...
    const payments = await tx`SELECT p.id,p.provider,p.environment,p.mode,p.state,p.purpose,p.provider_order_id,p.expected_minor,
      ...
      (SELECT jsonb_agg(jsonb_build_object('state',f.state,'expectedMinor',f.expected_minor,'actualMinor',f.actual_minor))
    ...
    const events = await tx`SELECT kind,created_at FROM booking_lifecycle_event WHERE order_id=${order.id} ORDER BY created_at,id`;
    let arrival = null;
    ...
    return { ...orderDTO(order), visits, arrival,
      ...
      events: events.map(row => ({ kind: row.kind, at: instant(row.created_at) })),
