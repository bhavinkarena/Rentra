...
export async function readBookingRecord(database, actor, orderId, env = process.env) {
  ...
  return database.begin(async tx => {
    ...
    return { ...orderDTO(order), visits, arrival,
      ...
      events: events.map(row => ({ kind: /^cancel_[a-f0-9]{32}$/.test(row.kind) ? 'visits_cancelled' : /^refund_[a-f0-9]{32}$/.test(row.kind) ? 'test_refund_processed' : row.kind, at: instant(row.created_at) })),
