...
export const ID_DOCUMENT_BY_ID = Object.fromEntries(ID_DOCUMENT_TYPES.map((d) => [d.id, d]));

export const MAX_DOC_BYTES = 2 * 1024 * 1024;

/**
 * The partner-application review promise, in hours.
 *
 * Mirrors `SLA_HOURS` in the API's admin queries, which is what actually
 * marks a row overdue. It is duplicated here only so the queue screen can
 * say "48 hours" in its copy without a round trip for a constant.
 */
