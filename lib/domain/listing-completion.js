...
export function listingCompletion(
  listing,
  { prices = [], amenities = [], photos = [], documents = [] } = {},
) {
  const l = listing ?? {};
  ...
  const sections = [
    ...
    },
    {
      ...
      note: l.exactAddress
        ? 'Exact address is hidden from guests until a booking is confirmed'
        : null,
      minutes: 3,
    },
    {
    ...
    },
    {
      ...
      note:
        amenities.length > 0 && amenities.length < 3
    ...
    },
    {
      ...
      note:
        photoList.length > 0 && photoList.length < MIN_PHOTOS
  ...
  return {
    ...
    review: {
      label:
        status === 'pending_verification' ? 'Verification visit' : 'Rentra reviews this property',
      hint:
