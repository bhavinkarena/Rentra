import {
  addLocalDays,
  consecutiveVisitDates,
  normalizeVisitDates,
  propertyToday,
} from './booking-dates.js';
...
export const DISCOVERY_INTENTS = [
  {
    ...
    slot: 'day',
    description:
      'Browse day visits. Select dates to check current hours, capacity and availability.',
  },
  {
    slug: 'with-pool',
    label: 'With pool',
    ...
    description:
      'Places with a swimming pool listed by the owner. Check pool rules on the listing.',
  },
  {
    slug: 'bonfire-allowed',
    label: 'Bonfire allowed',
    amenity: 'bonfire',
    description: 'Places with the bonfire amenity. Confirm seasonal restrictions with the owner.',
  },
  {
    ...
    amenity: 'open_lawn',
    description:
      'Explore places with open lawns for a possible shoot. Shoot permission and any additional charges require owner confirmation.',
  },
  {
    slug: 'corporate-offsite',
    label: 'Corporate offsite',
    amenity: 'banquet_lawn',
    description:
      'Explore places with banquet lawns. Confirm event permission, equipment and arrangements with the owner.',
  },
];
export const validRouteSlug = (value) =>
...
export const intentDiscoveryPath = (city, category, intent) =>
  `/${city}/${category}/intent/${intent}`;
...
export function resolveDiscoveryRoute(registry, segments) {
  ...
  if (!tail.length)
    return { city, category, path: base, title: `${category.name} in ${city.name}` };
  ...
  if (intent)
    return {
      ...
      category,
      intent,
      path: intentDiscoveryPath(citySlug, categorySlug, intent.slug),
      title: `${intent.label} · ${category.name} in ${city.name}`,
    };
  ...
  return area
    ? {
        ...
        category,
        area,
        path: areaDiscoveryPath(citySlug, categorySlug, area.slug),
...
export function parseDiscoveryQuery(input = {}, today = propertyToday()) {
  ...
  const value = (key) => {
    ...
    if (typeof raw !== 'string' || raw.length > 500) {
      errors.push(`Invalid ${key} filter.`);
      return '';
    }
    return raw.trim();
  };
  ...
  const number = (key, fallback, min, max) => {
    ...
    if (
      ...
      errors.push(`Choose ${key} between ${min} and ${max}.`);
      return fallback;
    }
    return Number(raw);
  };
  const filters = {
    ...
    guests: number('guests', 1, 1, 500),
    min: number('min', null, 0, 100000000),
    max: number('max', null, 0, 100000000),
    cancellation: choose('cancellation', ['flexible', 'moderate', 'strict'], ''),
    sort: choose('sort', Object.keys(SEARCH_SORTS), 'recommended'),
  ...
  const amenities =
    Array.isArray(amenityInput) &&
    amenityInput.length <= 10 &&
    amenityInput.every((v) => typeof v === 'string' && v.length <= 80)
      ? amenityInput.join(',')
      : value('amenities');
  if (amenities) {
    filters.amenities = [...new Set(amenities.split(','))];
    if (
      filters.amenities.length > 10 ||
      filters.amenities.some((a) => !/^[a-z0-9_]{1,80}$/.test(a))
    )
      errors.push('Choose at most 10 valid amenities.');
  }
  ...
  try {
    if (dates) filters.dates = normalizeVisitDates(dates.split(','));
      ...
      filters.dates =
        filters.mode === 'consecutive'
          ? consecutiveVisitDates(date, end || date)
          : normalizeVisitDates([date]);
    ...
    if (
      filters.mode === 'consecutive' &&
      filters.dates.some((d, i) => i && d !== addLocalDays(filters.dates[i - 1], 1))
    )
      errors.push('Consecutive mode requires consecutive dates.');
    if (filters.dates.some((d) => d < today || d > addLocalDays(today, 365)))
      errors.push('Choose dates within the next 365 days.');
  } catch {
    errors.push('Choose 1–10 valid, distinct visit dates.');
  }
  if (filters.min != null && filters.max != null && filters.min > filters.max)
    errors.push('Minimum budget must not exceed maximum budget.');
  for (const key of ['city', 'area', 'category'])
    if (filters[key] && !validRouteSlug(filters[key])) errors.push(`Choose a valid ${key}.`);
  ...
  return cards.sort((a, b) => {
    if (sort.startsWith('price_')) {
      if (a.price == null || b.price == null)
        return (a.price == null) - (b.price == null) || a.id.localeCompare(b.id);
      return (
        (sort === 'price_asc' ? a.price - b.price : b.price - a.price) || a.id.localeCompare(b.id)
      );
    }
    if (sort === 'newest')
      return String(b.createdAt).localeCompare(String(a.createdAt)) || a.id.localeCompare(b.id);
    return (
      Number(Boolean(b.badge === 'verified')) - Number(Boolean(a.badge === 'verified')) ||
