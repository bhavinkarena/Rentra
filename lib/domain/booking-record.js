...
export function bookingSummary(record) {
  ...
  for (const p of record.payments) for (const refund of p.refunds || []) lines.push(`${p.environment} refund ${refund.state}: requested ${bookingMoney(refund.expectedMinor)}; completed ${bookingMoney(refund.actualMinor)}`);
