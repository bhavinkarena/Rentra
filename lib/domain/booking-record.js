export const bookingMoney = (value) =>
  value == null
    ? 'Not recorded'
...
export function bookingSummary(record) {
  ...
  for (const v of record.visits)
    lines.push(
      `${v.reference} | ${v.date} | ${v.slot} | ${v.state} | ${v.guests} guests`,
  ...
  lines.push(
    '',
    `Accepted rent: ${bookingMoney(record.rentMinor)}; fee: ${bookingMoney(record.feeMinor)}; separate deposit: ${bookingMoney(record.depositMinor)}`,
    '',
    'PAYMENT RECORDS',
  );
  for (const p of record.payments)
    lines.push(
      `${p.provider} ${p.environment} | ${p.state} | ${p.providerOrderId || 'Provider reference pending'}`,
  ...
  for (const p of record.payments)
    for (const refund of p.refunds || [])
      lines.push(
        `${p.environment} refund ${refund.state}: requested ${bookingMoney(refund.expectedMinor)}; completed ${bookingMoney(refund.actualMinor)}${p.environment === 'test' ? '; actual bank refund: ₹0' : ''}`,
      );
  if (record.payments.some((p) => p.environment === 'test'))
    lines.push('TEST PAYMENT: no actual bank money was collected by the Test gateway.');
  lines.push(
    '',
    `Cancellation: ${record.policy.cancellationTier || 'Not recorded'}; policy ${record.policy.version || 'Not recorded'}`,
    ...record.policy.houseRules,
  );
