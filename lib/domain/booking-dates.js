...
export function propertyToday(now = new Date(), timeZone = BOOKING_POLICY.timeZone) {
  ...
  return new Date(instant.getTime() + KOLKATA_OFFSET_MINUTES * MINUTE_MS)
    .toISOString()
    .slice(0, 10);
}
...
export function normalizeVisitDates(dates) {
  ...
  if (new Set(dates).size !== dates.length)
    throw new RangeError('Duplicate visit dates are not allowed');
  return [...dates].sort();
}
...
export function propertyLocalInstant(date, time, timeZone = BOOKING_POLICY.timeZone) {
  assertTimeZone(timeZone);
  return new Date(
    parseLocalDate(date).getTime() + (minutesOfDay(time) - KOLKATA_OFFSET_MINUTES) * MINUTE_MS,
  ).toISOString();
}
