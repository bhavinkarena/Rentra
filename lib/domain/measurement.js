...
export const measurementSchema = z
  ...
  .strict()
  .refine((value) =>
    (value.source === 'browser' ? browserEvents : serverEvents).includes(value.event),
  );

export function measurementError(code) {
  if (['AVAILABILITY_CONFLICT', 'INVENTORY_CONFLICT'].includes(code)) return 'inventory_conflict';
  if (['QUOTE_CHANGED', 'QUOTE_EXPIRED', 'GATEWAY_VERSION_CONFLICT'].includes(code))
    return 'quote_changed';
  if (
    ['PAYMENTS_DISABLED', 'GATEWAY_CREDENTIALS_MISSING', 'PROVIDER_OUTCOME_UNKNOWN'].includes(code)
  )
