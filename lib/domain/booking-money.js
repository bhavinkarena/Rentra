...
export function parseINRMinor(value) {
  if (typeof value !== 'string' || !/^\d{1,12}(?:\.\d{1,2})?$/.test(value))
    throw new RangeError('Enter an INR amount with at most two decimal places');
...
export function calculateVisitPriceMinor({
  baseRentMinor,
  depositMinor,
  guests,
  includedGuests,
  capacity,
  extraGuestChargeMinor,
}) {
  ...
  const extraGuestsMinor = safeNumber(
    BigInt(Math.max(0, guests - includedGuests)) * BigInt(extraGuestChargeMinor),
  );
  ...
  const illustrativeAdvanceMinor = safeNumber(
    BigInt(applyBasisPoints(rentMinor, BOOKING_POLICY.illustrativeAdvanceBps)) + BigInt(feeMinor),
  );
  return {
...
export function priceVisitsMinor({ dates, slot, guests, rate, overridesByDate = {} }) {
  if (!BOOKING_SLOTS.includes(slot) || !rate)
    throw new RangeError('Slot pricing is not configured');
  ...
  const fields = [
    ...
    'brokerageMinor',
    'illustrativeAdvanceMinor',
    'illustrativeBalanceMinor',
  ];
  const totals = Object.fromEntries(
    fields.map((field) => [
      field,
      safeNumber(visits.reduce((sum, visit) => sum + BigInt(visit[field]), 0n)),
    ]),
  );
  return {
    currency: BOOKING_POLICY.currency,
    pricingVersion: BOOKING_POLICY.version,
    visits,
    totals,
  };
}
