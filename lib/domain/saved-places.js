import { z } from 'zod';
import { bookingSelectionSchema } from '../validation/zod/booking.js';
...
export const savedEntrySchema = z
  .object({
  ...
  .strict()
  .refine((v) => !v.selection || v.selection.rentableId === v.rentableId);
export const guestSavedSchema = z
  .array(savedEntrySchema)
  .max(SAVED_LIMIT)
  .refine(
    (v) =>
      new Set(v.map((e) => e.rentableId)).size === v.length &&
      new Set(v.map((e) => e.entryId)).size === v.length,
  );
export function validSavedSelection(value, rentableId, today = propertyToday()) {
  ...
  return parsed.success &&
    parsed.data.rentableId === rentableId &&
    parsed.data.dates.every((d) => d >= today)
    ? parsed.data
    : null;
}
export function parseGuestSaved(raw) {
  ...
  try {
    ...
    return parsed.success
      ? parsed.data.map((e) => ({
          ...e,
          selection: validSavedSelection(e.selection, e.rentableId),
        }))
      : [];
  } catch {
    return [];
...
export function savedListingHref(path, selection) {
  return selection
    ? `${path}?${new URLSearchParams({ dates: selection.dates.join(','), slot: selection.slot, guests: String(selection.guests) })}`
    : path;
}
export function selectionFromSavedUrl(search, rentableId) {
  const query = new URLSearchParams(search);
  if (!query.has('dates')) return null;
  return validSavedSelection(
    {
      rentableId,
      dates: query.get('dates').split(','),
      slot: query.get('slot'),
