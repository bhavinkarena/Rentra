...
export async function savedPlaceCards(database, entries) {
  ...
  const rows = await database`SELECT r.id,r.title,r.slug,r.public_code,a.name AS area,c.name AS city
  ...
  return entries.map(e => {
    ...
    return row ? { rentableId:e.rentableId, selection, available:true, title:row.title,
      area:`${row.area}, ${row.city}`, href:savedListingHref(listingPath(row.slug,row.public_code),selection) }
