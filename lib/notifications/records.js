...
export async function retryNotification(database, adminId, id) {
  ...
  return database.begin(async tx => {
    ...
    await tx`INSERT INTO audit_log(actor_type,actor_id,entity_type,entity_id,action) VALUES('admin',${adminId},'notification_outbox',${id},'notification_retry')`;
  });
}
export async function reconcileUnknownNotification(database, adminId, id, sid, options = {}) {
  ...
  await database.begin(async tx => {
    await admin(tx, adminId);
    await tx`UPDATE notification_outbox SET provider_id=${outcome.id},state=${outcome.state},failure_code=NULL,next_attempt_at=clock_timestamp(),
      delivered_at=CASE WHEN ${outcome.state === 'delivered'} THEN clock_timestamp() ELSE delivered_at END WHERE id=${id} AND state='unknown'`;
    await tx`INSERT INTO audit_log(actor_type,actor_id,entity_type,entity_id,action) VALUES('admin',${adminId},'notification_outbox',${id},'notification_reconciled')`;
  });
}
