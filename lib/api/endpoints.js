...
/** Public. Safe to cache — no session is read on any of these. */
export const discoveryApi = {
  listings: (query) => api.get(`/discovery/listings${qs(query)}`),
  nearby: (query) => api.get(`/discovery/listings/nearby${qs(query)}`),
  search: (query) => api.get(`/discovery/search${qs(query)}`),
  registry: () => api.get('/discovery/registry'),
  routeCount: (path) => api.get(`/discovery/route-count${qs({ path })}`),
  cities: () => api.get('/discovery/cities'),
  areas: (citySlug) => api.get(`/discovery/cities/${citySlug}/areas`),
  areaCount: (citySlug, areaSlug) =>
    api.get(`/discovery/cities/${citySlug}/areas/${areaSlug}/count`),
  sitemap: () => api.get('/discovery/sitemap'),
  listing: (code) => api.get(`/discovery/listings/${code}`),
  nextDates: (code) => api.get(`/discovery/listings/${code}/next-dates`),
  similar: (id, query) => api.get(`/discovery/listings/${id}/similar${qs(query)}`),
  /**
  ...
  availability: (code, query) =>
    api.get(`/discovery/listings/${code}/availability${qs(query)}`, { cache: 'no-store' }),
};
...
export const partnerApi = {
  ...
  savePriceOverride: (id, form) => api.post(`/partner/listings/${id}/calendar/price-override`, form),
