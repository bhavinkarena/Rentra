...
import {
  ...
  date, jsonb, real, geometry, uniqueIndex, index, primaryKey, check,
...
export const booking = pgTable(
  'booking',
  {
    ...
    /* ----------------------------------------------------------------------
       ...
       Every column here is additive and nullable (or defaulted), because 44
  ...
  (t) => [
    ...
    check('booking_minor_amounts_nonnegative_chk',
      sql`(${t.amountRentMinor} IS NULL OR ${t.amountRentMinor} >= 0)
        AND (${t.amountFeeMinor} IS NULL OR ${t.amountFeeMinor} >= 0)
        AND (${t.amountDepositMinor} IS NULL OR ${t.amountDepositMinor} >= 0)
        AND (${t.amountAdvanceMinor} IS NULL OR ${t.amountAdvanceMinor} >= 0)
        AND ${t.collectedMinor} >= 0`),
