...
export async function lockCustomerAccount(tx, session, env = process.env) {
  if (session?.role !== 'customer' || !z.string().uuid().safeParse(session.userId).success
    ...
    throw new CustomerAccountError('Please log in to your customer account again.');
  }
  ...
  if (!active) throw new CustomerAccountError('Please log in to your customer account again.');
  return user;
}
