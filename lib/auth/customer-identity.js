...
export async function requestCustomerCode(database, { phone: input, browserToken, ip = 'unknown' }, { env = process.env, deliver = deliverCustomerCode } = {}) {
  ...
  const inserted = await database.begin(async (tx) => {
    ...
    await tx`INSERT INTO customer_otp_challenge(id,phone,browser_hash,code_hash,delivery_mode,expires_at)
...
export async function verifyCustomerCode(database, { phone: input, challengeId, browserToken, code, ip = 'unknown' }, { env = process.env } = {}) {
  const phone = customerPhone.parse(input);
