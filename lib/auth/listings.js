...
import { fieldErrors } from '@/lib/validation/zod';
import {
  basicsSchema, locationSchema, capacitySchema, rulesSchema,
  pricingSchema, termsSchema, ownershipDocSchema,
} from '@/lib/validation/zod/listing';
import { listingCompletion, MIN_PHOTOS, MAX_PHOTOS } from '@/lib/domain/listing-completion';
import { movePhoto, photoId, renumberPhotos } from '@/lib/domain/listing-photos';
import { getListingForEdit } from '@/lib/db/listing-queries';
import { revalidateListing } from '@/lib/cache/listing-cache';
import {
  uploadPrivateDocument, detectMime, UPLOAD_LIMITS, isCloudinaryConfigured,
...
export async function createListingDraft() {
  ...
  /**
   * categoryId / cityId / areaId are NOT NULL, so a fresh draft borrows the
   * first of each and the builder overwrites them. Making them nullable is the
   * alternative, but then a LIVE listing could exist with no city — and the
   * whole SEO route tree is built on city and area, so that is the worse
   * trade. The builder's `location` section is what makes them real.
   */
  const [[cat], [firstCity], [firstArea]] = await Promise.all([
    db.select({ id: category.id }).from(category).limit(1),
    db.select({ id: city.id }).from(city).limit(1),
    db.select({ id: area.id }).from(area).limit(1),
  ]);

  if (!cat || !firstCity || !firstArea) {
    // Better than a foreign-key error three frames deep.
    throw new Error('No categories, cities or areas seeded — run `npm run db:seed` first.');
  }
  ...
  const [row] = await db
    ...
    .values({
      clientId: user.id,
      // Placeholders so the row is valid; the builder fills them in.
  ...
  redirect(`/partner/listings/${row.id}`);
}
...
export async function savePricing(_prev, formData) {
  ...
  await db.delete(rentablePrice).where(eq(rentablePrice.rentableId, listing.id));
  if (rows.length) await db.insert(rentablePrice).values(rows);

  // Price is a FREE field — a live listing stays live.
  await db.update(rentable).set({ updatedAt: new Date() }).where(eq(rentable.id, listing.id));
