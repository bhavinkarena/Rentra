...
export default function BookingPriceBox({ rentableId, listingTitle }) {
  ...
  return <div id="booking-summary" className="scroll-mt-24 rounded-lg border border-border bg-card p-5 shadow-sm">
    <h2 className="text-h3">Price for your visit</h2>
    <label className="mt-4 block text-meta font-semibold">Guests per visit<input type="number" min="1" max="500" disabled={!selectionReady} value={guests} onChange={event => setGuests(event.target.value === '' ? '' : Number(event.target.value))} className="mt-1 min-h-11 w-full rounded-md border border-border px-3" /></label>
    <QuoteSummary />
