        ...
        <>
          ...
          <dl className="mt-4 space-y-3 text-meta">
            {Object.entries({
              ...
              'Total for selected visits': quote.totals.totalMinor,
            }).map(([label, amount]) => (
              <div key={label} className="flex justify-between gap-3">
                <dt>{label}</dt>
                <dd
                  className="font-semibold tabular"
                  data-quote-total={label === 'Total for selected visits' ? '' : undefined}
                >
                  {formatINRMinor(amount)}
                </dd>
              </div>
            ))}
          </dl>
          <p className="mt-4 rounded-md bg-ink-50 p-3 text-meta">
