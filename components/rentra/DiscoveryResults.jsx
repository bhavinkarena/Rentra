  ...
  return <section className="mx-auto max-w-(--container-page) px-4 py-10 sm:px-6">
    ...
    {failed ? <div role="alert" className="mt-6"><h2 className="text-h3">Search is temporarily unavailable</h2><p>Your filters are in the address bar. Please try again.</p><a className="underline" href={href({ page: filters.page })}>Retry search</a></div> : !messages.length && <>
      <div className="mt-8 flex flex-wrap items-center justify-between gap-3"><h2 className="text-h3">{result.total} {result.total === 1 ? 'place' : 'places'}{filters.dates.length ? ' matching every selected date' : ''}</h2>
        <form action={path}><input type="hidden" name="q" value={filters.q}/><input type="hidden" name="city" value={filters.city}/><input type="hidden" name="area" value={filters.area}/><input type="hidden" name="category" value={filters.category}/><input type="hidden" name="mode" value={filters.mode}/><input type="hidden" name="dates" value={filters.dates.join(',')}/><input type="hidden" name="slot" value={filters.slot}/><input type="hidden" name="guests" value={filters.guests}/><input type="hidden" name="min" value={filters.min ?? ''}/><input type="hidden" name="max" value={filters.max ?? ''}/><input type="hidden" name="cancellation" value={filters.cancellation}/>{filters.amenities.map(value => <input key={value} type="hidden" name="amenities" value={value}/>)}<label className="text-meta font-medium">Sort <select name="sort" defaultValue={filters.sort} className="ml-2 min-h-11 rounded-md border border-border bg-white px-3">{Object.entries(SEARCH_SORTS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button className="ml-2 min-h-11 text-brand-700 underline">Apply</button></form>
      </div>
