...
export default async function DiscoveryResults({ query, registry, route = null }) {
  ...
  return <section className="mx-auto max-w-(--container-page) px-4 py-10 sm:px-6">
    ...
    <div className="mt-4 flex flex-wrap gap-2" role="group" aria-label="Active filters">{chips.map(([key, label]) => <Link key={key} className="inline-flex min-h-10 items-center rounded-full bg-brand-50 px-3 text-meta text-brand-900" href={href(clearChip(key))} aria-label={`Remove ${label}`}>{label} ×</Link>)}</div>
