  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
      <header>
        ...
        <p className="mt-2">
          Each order keeps its original property details and individual visit statuses. Mixed orders
          can appear in more than one tab.
        </p>
      </header>
      <form action={base} className="flex flex-wrap items-end gap-3">
        <input type="hidden" name="tab" value={data.tab} />
        <label className="min-w-0 flex-1">
          Search property or booking reference
          <input
        ...
        <button className="min-h-11 rounded-md bg-brand-700 px-4 text-white">Search</button>
      </form>
      <nav aria-label="Booking history filters" className="flex flex-wrap gap-4">
        {['all', 'upcoming', 'past', 'cancelled'].map((tab) => (
          <Link
            key={tab}
            aria-current={data.tab === tab ? 'page' : undefined}
            className={linkClass + (data.tab === tab ? ' font-bold' : '')}
      ...
      <ul className="space-y-4">
        {data.items.map((item) => (
          <li key={item.id} className="space-y-2 rounded-lg border border-border p-4">
            <Link className={linkClass + ' text-h3'} href={`${base}/${item.id}`}>
              {item.title}
            </Link>
            <p className="break-all text-meta">{item.reference}</p>
            <div className="flex flex-wrap gap-2">
              <span className={badge}>Booking: {item.state}</span>
            ...
            <p>
              ...
              {item.visitStates.join(', ') || 'No visits recorded'}
            </p>
      ...
      <nav aria-label="Booking pages" className="flex flex-wrap items-center gap-5">
        {data.page > 1 ? (
    ...
    <article className="mx-auto max-w-4xl space-y-6 break-words p-4 sm:p-6">
      ...
      <header>
        <h1 className="text-h1">
          {confirmed ? (test ? 'Test booking record' : 'Booking record') : 'Booking status'}
        </h1>
        <h2 className="mt-2 text-h3">{record.title}</h2>
        <p className="mt-2 break-all text-meta">{record.reference}</p>
        <p className="mt-3">
          <span className={badge}>Booking: {record.state}</span>
        </p>
      </header>
      {test ? (
      ...
      {!operational ? (
        <p>
          <Link className={linkClass} href={`/bookings/${record.id}/reviews`}>
            Review visits and see review status
          </Link>
        </p>
      ) : (
        <p>
      ...
      <section>
        <h2 className="text-h3">Accepted price</h2>
        <p>
      ...
      <section>
        <h2 className="text-h3">Private arrival details</h2>
        {record.arrival ? (
          ...
          {record.payments.flatMap((p) =>
            (p.refunds || []).map((refund, index) => (
              <p key={`${p.id}-${index}`}>
                ...
                {refund.providerRefundId ? ` · provider reference: ${refund.providerRefundId}` : ''}
                {refund.needsReview
      ...
      <section>
        <h2 className="text-h3">Order timeline</h2>
