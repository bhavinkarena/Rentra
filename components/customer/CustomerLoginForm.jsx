...
export default function CustomerLoginForm() {
  ...
  return <div className="mt-8 space-y-6">
    <form action={send} className="space-y-4">
      <label htmlFor="phone" className="block text-meta font-semibold">Mobile number</label>
      <Input id="phone" name="phone" type="tel" autoComplete="tel" required maxLength={18} placeholder="98765 43210" readOnly={Boolean(sent?.challengeId)} defaultValue={sent?.phone ?? ''} />
      {sent?.error ? <p role="alert" className="text-meta text-danger">{sent.error}</p> : null}
      <Button type="submit" size="lg" className="w-full" disabled={sending || verifying || seconds > 0}>{sending ? 'Requesting code…' : seconds > 0 ? `Resend in ${seconds}s` : sent?.challengeId ? 'Resend code' : 'Send code'}</Button>
    </form>
    {sent?.challengeId ? <form action={verify} className="space-y-4" key={sent.challengeId}>
      <p role="status" className="text-meta text-ink-600">{sent.development ? 'Development login: enter 123456. No SMS is sent.' : `Code requested for +91 ••••••${sent.phone.slice(-4)}. It expires in 5 minutes.`}</p>
      <input type="hidden" name="phone" value={sent.phone} />
      <label htmlFor="code" className="block text-meta font-semibold">One-time code</label>
