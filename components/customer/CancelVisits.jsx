...
export default function CancelVisits({record}) {
  ...
  return <div className="space-y-6"><Link className="inline-flex min-h-11 items-center text-brand-700 underline" href={`/bookings/${record.id}`}>Back to booking record</Link><h1 className="text-h1">{receipt?'Visits cancelled':'Cancel selected visits'}</h1><h2 className="text-h3">{record.title}</h2>
    ...
    <section className="border-t border-border pt-4"><h2 className="text-h3">Need different dates or guests?</h2><p className="mt-2">Visits cannot be edited in place. For unstarted visits, review cancellation costs first, then cancel and make a new booking at current availability and prices. A replacement is not reserved or guaranteed. For a started visit, contact the host from your booking record. Structured support requests follow in a later release.</p></section>
  </div>;
}
