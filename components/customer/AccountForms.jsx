...
const control =
  'mt-1 min-h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-body';
const button =
  'min-h-11 rounded-md bg-brand-600 px-5 py-2 font-semibold text-white disabled:opacity-50';
  ...
  const preferences = (
    <div className="space-y-5">
      ...
      <p id="email-note" className="text-meta text-ink-600">
        Email is optional and is not used for login or message delivery.{' '}
      ...
      <p className="text-meta text-ink-600">
        This saves your preference. The app currently displays English.
      </p>
...
export function PhoneChangeForm() {
  ...
  return (
    <div className="space-y-6">
      ...
      {request.challengeId ? (
        <form key={request.challengeId} action={verifyAction} className="space-y-4">
          <p className="text-meta">
            Code requested for +91 {request.phone}. It expires in five minutes. Resend after 60
            seconds.
          </p>
          {request.development ? (
            <p role="status" className="rounded-md bg-brand-50 p-3 text-meta">
              Development mode: use 123456. No SMS was sent.
            </p>
          ) : null}
