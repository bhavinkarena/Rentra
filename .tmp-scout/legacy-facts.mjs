...
const show = (title, rows) => {
  console.log(`\n### ${title}`);
  console.table(rows.length ? rows : [{ result: 'none' }]);
};

show(
  'guests above listing capacity',
  await sql`
...
show(
  'same listing + day + slot booked more than once',
  await sql`
...
show(
  'day+night pair on the same listing/date (a latent full-day overlap)',
  await sql`
...
show(
  'listing status behind each legacy booking',
  await sql`
  select r.status, count(*)::int bookings from booking b join rentable r on r.id=b.rentable_id group by 1`,
);

show(
  'booking dates vs the availability window (2026-09-06..2026-12-10)',
  await sql`
...
show(
  'legacy booking rows that have a matching availability row',
  await sql`
...
show(
  'does a price row exist for every booked listing+slot, and does it match?',
  await sql`
...
show(
  'customer identity behind legacy bookings',
  await sql`
...
show(
  'fee reconciles to the 8% platform fee?',
  await sql`
...
show(
  'advance = 25% rent + full fee (the illustrative rule)?',
  await sql`
...
show(
  'payout status distribution (is any seeded money marked paid?)',
  await sql`
  select status, count(*)::int n, sum(net)::bigint net_rupees from payout group by 1`,
);

show(
  'check_in_from / check_out_by distinct values (are any machine-parseable?)',
  await sql`
  select check_in_from, check_out_by, count(*)::int listings from rentable group by 1,2`,
);

show(
  'slot coverage in rentable_price vs slots actually booked',
  await sql`
