...
while ((match = tag.exec(body))) {
  ...
  if (!closing) {
    stack.push({ name: lower, line });
    continue;
  }
  const top = stack.pop();
  if (!top) problems.push(`line ${line}: </${lower}> with nothing open`);
  else if (top.name !== lower)
    problems.push(`line ${line}: </${lower}> closes <${top.name}> opened at line ${top.line}`);
}
...
for (const bad of body.matchAll(/&(?!#?[a-zA-Z0-9]+;)/g)) {
  problems.push(
    `bare ampersand at offset ${bad.index}: ${JSON.stringify(body.slice(bad.index, bad.index + 24))}`,
  );
}
...
console.log(
  `tags balanced over ${stack.length === 0 ? 'whole body' : 'partial body'} · ${tasks.length} checklist tasks · ${ids.size} ids`,
);
if (problems.length) {
