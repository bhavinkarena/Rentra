...
for (const [kind, url] of [
  ...
  await step('list databases', async () =>
    (await admin`select datname from pg_database where not datistemplate order by 1`)
      .map((r) => r.datname)
      .join(', '),
  );
...
await step(
  'create table with gist exclusion',
  () => probe`
...
await step(
  'insert night visit 18:00 -> 10:00 next day',
  () => probe`
  insert into probe_range values (${LISTING}, 'active', tstzrange('2026-10-01 18:00+05:30','2026-10-02 10:00+05:30','[)'))`,
);
await step(
  'TOUCHING day visit starting exactly 10:00 must be ACCEPTED (half-open)',
  () => probe`
...
await step(
  'released row may overlap (partial constraint)',
  () => probe`
