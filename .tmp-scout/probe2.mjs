...
const open = (url, db) =>
  postgres(db ? url.replace(/\/[^/?]+(\?|$)/, `/${db}$1`) : url, {
    prepare: false,
    max: 1,
    onnotice: () => {},
    connect_timeout: 20,
...
async function expect(label, run, wantCode) {
  try {
    const v = await run();
    if (wantCode) {
      console.log(`FAIL  ${label} -> expected error ${wantCode}, got success`);
      fails++;
      return v;
    }
    ...
    if (wantCode && e.code === wantCode) {
      console.log(`PASS  ${label} -> rejected with ${e.code}`);
      return null;
    }
...
await expect('reservation ledger with partial gist exclusion', () =>
  p.unsafe(`
...
const ins = (state, a, b) =>
  p.unsafe(
    `insert into probe_res (rentable_id, state, occupied) values ($1,$2,tstzrange($3,$4,'[)'))`,
    [L, state, a, b],
  );
await expect('night visit 18:00 -> 10:00 next day held', () =>
  ins('held', '2026-10-01 18:00+05:30', '2026-10-02 10:00+05:30'),
);
await expect('TOUCHING day visit starting exactly 10:00 accepted (half-open)', () =>
  ins('held', '2026-10-02 10:00+05:30', '2026-10-02 18:00+05:30'),
);
await expect(
  'OVERLAPPING 09:00-18:00 rejected by exclusion',
  () => ins('held', '2026-10-02 09:00+05:30', '2026-10-02 18:00+05:30'),
  '23P01',
);
await expect('released row may overlap (partial predicate)', () =>
  ins('released', '2026-10-02 09:00+05:30', '2026-10-02 18:00+05:30'),
);
await expect('different listing may overlap', () =>
  p.unsafe(
...
await p.end().catch(() => {});
await expect('drop disposable database with force', () =>
  admin.unsafe(`drop database ${NAME} with (force)`),
);
await expect('confirm gone', async () =>
  (await admin`select count(*)::int n from pg_database where datname=${NAME}`)[0].n === 0
    ? 'gone'
    : 'STILL PRESENT',
);
