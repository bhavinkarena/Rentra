...
export default async function Report({params}) {
 ...
 try{review=await customerApi.review(id);}
 catch(e){if(e instanceof ApiError&&[400,403,404,422].includes(e.status))notFound();throw e;}
 return <section className="mx-auto max-w-2xl space-y-5 p-4"><h1 className="text-h1">Report review or reply</h1><p className="break-words">{review.body}</p>{review.ownerReply?<p>Owner reply: {review.ownerReply}</p>:null}<p>Tell us which content violates the review rules and why. A report is saved for staff review; it does not automatically remove content.</p><ReviewControl kind="report" id={id}/></section>;
}
