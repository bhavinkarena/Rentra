...
export default async function CustomerLayout({children}) {
  ...
  return <div className="min-h-screen bg-ink-25">
    ...
    <header className="border-b border-border bg-background"><div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-2 px-4 py-3"><Link href="/" aria-label="Rentra home"><RentraLogo className="h-7 w-auto"/></Link><CustomerNavigation/></div></header>
    <main id="customer-content" className="mx-auto max-w-3xl px-4 py-8 sm:py-12">{children}</main>
  </div>;
}
