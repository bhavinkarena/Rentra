...
async function document(params) {
  ...
  const selected = version?.[0] || POLICY_VERSION,
    content =
      Object.hasOwn(policyVersions, selected) && Object.hasOwn(policyVersions[selected], kind)
        ? policyVersions[selected][kind]
        : null;
  if (!content) notFound();
  ...
  return (
    <article className="mx-auto max-w-3xl space-y-7 px-4 py-10">
      ...
      <header>
        ...
        <p className="mt-3">
          Version {selected} · Published {selected}
        </p>
